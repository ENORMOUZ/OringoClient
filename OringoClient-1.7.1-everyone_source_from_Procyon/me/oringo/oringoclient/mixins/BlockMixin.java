...
public abstract class BlockMixin
{
    ...
    @Shadow
    public abstract AxisAlignedBB getSelectedBoundingBox(final World p0, final BlockPos p1, final IBlockState p2);
    ...
    public void func_180638_a(final World worldIn, final BlockPos pos, final IBlockState state, final AxisAlignedBB mask, final List<AxisAlignedBB> list, final Entity collidingEntity) {
        final BlockBoundsEvent event = new BlockBoundsEvent((Block)this, this.getSelectedBoundingBox(worldIn, pos, state), pos, collidingEntity);
        if (MinecraftForge.EVENT_BUS.post((Event)event)) {
            return;
        }
        if (event.aabb != null && mask.intersects(event.aabb)) {
            list.add(event.aabb);
        }
    }
}
