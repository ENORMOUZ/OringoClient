...
public class AutoSumoBot extends Module
{
    ...
    public static int smartStrafeOrRandomV2(final Entity target) {
        ...
        if (target != null) {
            ...
            while (i < 100) {
                final int angle = 60;
                if (checkBlocksBelow(mc.player.posX - Math.sin(Math.toRadians(target.rotationYaw - angle) * 0.13 * i), mc.player.posY, mc.player.posZ + Math.cos(Math.toRadians(target.rotationYaw - angle)) * 0.13 * i)) {
                    if (checkBlocksBelow(mc.player.posX - Math.sin(Math.toRadians(target.rotationYaw + angle) * 0.13 * i), mc.player.posY, mc.player.posZ + Math.cos(Math.toRadians(target.rotationYaw + angle)) * 0.13 * i)) {
                        return -1;
                    }
            ...
            if (checkBlocksBelow(target.posX - Math.sin(Math.toRadians(mc.player.rotationYaw) * 3.0), target.posY, target.posZ + Math.cos(Math.toRadians(mc.player.rotationYaw)) * 3.0)) {
